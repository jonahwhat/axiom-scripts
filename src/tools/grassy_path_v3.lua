...
-- Check how many grass blocks that can see air are around x,y,z
if (getBlockState(x,y,z-1) == grass and not isSolid(getBlock(x,y+1,z-1))) or (getBlockState(x,y+1,z-1) == grass and not isSolid(getBlock(x,y+2,z-1))) or (getBlockState(x,y-1,z-1) == grass and not isSolid(getBlock(x,y,z-1))) then
...
if (getBlockState(x-1,y,z) == grass and not isSolid(getBlock(x-1,y+1,z))) or (getBlockState(x-1,y+1,z) == grass and not isSolid(getBlock(x-1,y+2,z))) or (getBlockState(x-1,y-1,z) == grass and not isSolid(getBlock(x-1,y,z))) then
    blocks_surrounding = blocks_surrounding + 1
    west = true
end
